...
st.set_page_config(page_title="Representation Disentanglement RL", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 Representation Disentanglement RL</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Exogenous vs endogenous factor separation | Causal representation learning for time series</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Disentanglement RL")
...
st.sidebar.markdown(f"**Latent dim:** {config.LATENT_DIM} | **Exo/Endo:** {config.EXOGENOUS_DIM}/{config.ENDOGENOUS_DIM}")
st.sidebar.markdown(f"**Seq len:** {config.SEQ_LEN} | **β:** {config.BETA}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'disentanglement_rl_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Disentanglement: {etf['disentangle_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Disentanglement"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Disentanglement", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🧠 Top ETFs by Disentanglement Quality (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Representation disentanglement** separates exogenous (macro) and endogenous (market) factors.
        - The RL agent learns a disentangled representation of the return-generating process.
        - **Exogenous factors** = macro-driven variations (VIX, DXY, yields).
        - **Endogenous factors** = market-driven variations (momentum, mean-reversion).
        - The score is the **disentanglement quality** – higher = better factor separation.
        - High quality → better causal representation → better prediction.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the disentanglement scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Representation Disentanglement RL | Causal representation learning for time series")
