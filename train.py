...
from disentanglement_rl import disentanglement_rl_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        s = disentanglement_rl_score(
            ret_window[ticker].values,
            macro_window,
            latent_dim=config.LATENT_DIM,
            exo_dim=config.EXOGENOUS_DIM,
            endo_dim=config.ENDOGENOUS_DIM,
    ...
    top_etfs = [{"ticker": t, "disentangle_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/disentanglement_rl_{timestamp}.json"
